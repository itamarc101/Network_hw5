...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # * Fill in start (1)
        server_socket.bind((host,port)) # listens to the ip and port that wants to connect (recieve data on this socket)
        server_socket.listen()          # incoming connections that are waiting in the queue
        # * Fill in end (1)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            data = client_socket.recv(8192) # recieve data packet size of 8192 bytes 
            ...
            try:
                ...
                # * Fill in start (4)
                client_socket.sendall(response) # sends the response when all the data has been sent (sends the entire buffer that passed)
